...
// Get an element from a map, or return a default value
template <typename T, typename U>
T getOrDefault(const std::map<U, T>& map, const U& key, const T& defaultValue) {
...
public:
	...
	std::pair<polyType, std::vector<polyType>> optimize(int level=1, int verbosity=1, int maxIters=-1) {
        ...
        // Check if there are any binary variables
        ...
        // The parameterized equality constraints for the binary variables
        if (hasBinary) {
		...
		while (toProcess.size() > 0) {
            ...
            if (!isLin) {
                ...
                // If we have binary vars, these should be branched with equalities
                if (hasBinary) {
